...
import {faArrowDown,faBookReader, faCloud, faEdit, faEye, faL, faTrash} from '@fortawesome/free-solid-svg-icons'
...
import { Commentary, Story, StoryTelling } from 'src/app/private/models/Entity';
import { AlreadyRated, ChapterFacadeDto, CommentaryDto, StoryBoxesDto, userDisplay } from 'src/app/private/models/EntityDto';
...
private ParamRoute$:string;
  ...
  constructor(private config: NgbRatingConfig,
    private storyBoxApiQuery:HttpApiQueryService<StoryBoxesDto>,
    private storyBoxChapterApiQuery:HttpApiQueryService<ChapterFacadeDto>,
    private ratingApiQuery:HttpApiQueryService<AlreadyRated>,
    private ratingApiCommand:HttpApiCommandService<AlreadyRated>,
...
return 
  ...
  innitRateForm(){
    this.RatingForm =  this.formBuilder.group({
      id: [this.StoryBoxe.idStoryTell,[Validators.required,Validators.min(0)]],
      rate: [this.selected,[Validators.required]]
    });
  }
...
async getStoryRating(){
  ...
  const response=this.ratingApiQuery.getWithDetails(endpoint,this.StoryBoxe.idStoryTell.toString());
  this.Rating=await lastValueFrom(response);
  this.selected=this.Rating.rated;
  console.log(this.Rating.totalRate);
}
  ...
  async selectRate(){ if(this.Rating.rated==this.selected)return;
    this.innitRateForm();
    if(this.Rating.alreadyRated){await this.updateRating();}
    else{await this.createRating();}
    await this.updateRatingStory();
  }

 async createRating(){
  const endpoint="Rating";
    const response= this.ratingApiCommand.post(this.RatingForm.value,endpoint);
    const result=await lastValueFrom(response); 
    await this.getStoryRating();
  }
 ...
 async updateRating(){
    const endpoint="Rating";
    const response= this.ratingApiCommand.put(this.RatingForm.value,endpoint,this.StoryBoxe.idStoryTell.toString());
    const result=await lastValueFrom(response); 
    await this.getStoryRating();
  }

  async updateRatingStory(){
    const endpoint="StoryTelling/Rating"
    const response=this.storyTellApiCommand.putSpec(endpoint,this.StoryBoxe.idStoryTell.toString());
    const result=await lastValueFrom(response);
  }
